...
## How used
Claude Code drove every stage of planning so far: running mechanic-comparison and time-budget sanity checks through the Fable advisor, writing project documentation (`CLAUDE.md`, the `Notes.md` design lock, an internal GDD, and submission-note skeletons), and setting up the GitHub repo. It also drafted an initial C# scaffold (data classes + a game-state manager) that got rolled back after a scope correction. Unity-MCP is installed and configured but not yet exercised — live Editor/scene work is still ahead.
